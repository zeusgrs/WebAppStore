<?php
class Plugins {
    public function Youtube($videoId,$width = "565",$height = "415") {
    ...
    public function YoutubeFullLink($link,$width = "565",$height = "415") {
        if($width == NULL && $height != NULL) {$width = ($height*1.5)."px";}
        if($height == NULL && $width != NULL) {$height = ($width/1.5)."px";}
    ...
    public function VimeoFullLink($link,$width = "565",$height = "415") {
        if($width == NULL && $height != NULL) {$width = ($height*1.5)."px";}
        if($height == NULL && $width != NULL) {$height = ($width/1.5)."px";}
    ...
    public function Image($file,$width = "565",$height = "415") {
        if($width == 0){ $width="";} else {$width="width='$width'";}
        if($height == 0){ $height="";} else {$height="height='$height'";}
        return "<img src='$file'  $width $height>\n";
    }

    public function ImageLightbox($file,$width = "565",$height = "415",$group) {
        if($width == 0){ $width="";} else {$width="width='$width'";}
        if($height == 0){ $height="";} else {$height="height='$height'";}
        return "<a href='$file' data-lightbox='$group'><img src='$file'  $width $height></a>\n";
    }
